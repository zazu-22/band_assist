...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (pathBandId !== tokenData.band_id) {
      return new Response(JSON.stringify({
        error: 'File does not belong to the authorized band'
      }), {
        status: 403,
        headers: {
          ...corsHeaders,
    ...
    // Allow re-use within grace period - skip marking as used again
      ...
      const { data: updateResult, error: updateError } = await supabase.from('file_access_tokens').update({
        used_at: new Date().toISOString()
      }).eq('id', tokenData.id).is('used_at', null) // Only update if still unused (atomic CAS)
      .select().single();
      if (updateError || !updateResult) {
        // Token was already used by another concurrent request - check grace period
        const { data: recheckToken, error: recheckError } = await supabase.from('file_access_tokens').select('used_at').eq('id', tokenData.id).single();
    ...
    const { data, error } = await supabase.storage.from('band-files').download(storagePath);
    if (error) {
