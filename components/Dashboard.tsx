    ...
    <div className="p-6 lg:p-10 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl relative overflow-hidden group">
          ...
          <p className="text-zinc-400 text-sm font-medium uppercase tracking-wider">Total Setlist</p>
          <p className="text-4xl font-bold text-white mt-2">{totalSongs} <span className="text-lg text-zinc-500 font-normal">Songs</span></p>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl relative overflow-hidden group">
          ...
          {nextGig ? (
              <>
                <p className="text-xl font-bold text-white mt-2 truncate" title={nextGig.title}>{nextGig.title}</p>
                <p className="text-sm text-zinc-500">{new Date(nextGig.date).toLocaleDateString(undefined, {month:'short', day:'numeric'})} {nextGig.location ? `• ${nextGig.location}` : ''}</p>
              </>
          ) : (
      ...
      <div className="grid lg:grid-cols-3 gap-8">
              ...
              >
                ...
                <div className="flex items-center gap-3">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium border ${
                        song.status === 'Performance Ready' ? 'bg-green-950/30 text-green-500 border-green-900' :
                        song.status === 'In Progress' ? 'bg-blue-950/30 text-blue-500 border-blue-900' :
        ...
        <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 border border-zinc-700 rounded-2xl p-6 text-white relative overflow-hidden">
            <div className="relative z-10">
                ...
                <p className="text-zinc-300 italic mb-6">"You can't lose with the blues."</p>
                <div className="bg-black/30 p-4 rounded-xl backdrop-blur-sm border border-white/10">
                    <p className="text-sm text-zinc-300 font-mono">Daily Tip: When playing La Grange, keep that right hand loose for the shuffle. It's all in the wrist.</p>
                </div>
            </div>
            {/* Decorative Texture */}
