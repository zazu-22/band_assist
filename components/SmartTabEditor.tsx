...
import { Edit3, Eye, MessageSquare, X, Check, ArrowUp, ArrowDown, Type } from 'lucide-react';
...
export const SmartTabEditor: React.FC<SmartTabEditorProps> = ({ 
  ...
  const analyzeLine = (line: string): LineType => {
    ...
    const chordCount = tokens.filter(t => validChordPattern.test(t) && !commonWords.includes(t.toUpperCase())).length;
    ...
    return (
      <div className="font-mono text-sm relative min-h-[400px] bg-zinc-900 rounded-xl p-4 overflow-x-auto">
        {lines.map((originalLine, idx) => {
            ...
            const lineToRender = (type === 'CHORD' && transposeLevel !== 0) 
            ...
            else if (type === 'TAB') className += "text-cyan-400 leading-none tracking-tighter opacity-90";
            else if (type === 'HEADER') className += "text-white font-bold text-lg mt-4 mb-2 bg-zinc-800/50 inline-block px-3 py-1 rounded-lg border border-zinc-700";
            ...
            return (
                <div key={idx} className="group relative hover:bg-zinc-800/30 transition-colors flex">
                    ...
                    {!readOnly && (
                        <div className="w-8 shrink-0 text-zinc-700 text-[10px] select-none flex items-center justify-center opacity-0 group-hover:opacity-100 cursor-pointer"
                             onClick={() => {
                    ...
                    {annotation && (
                        <div className="absolute right-4 top-0 z-10">
                            <div className="bg-yellow-200/10 backdrop-blur-md border border-yellow-500/50 text-yellow-200 text-xs px-3 py-1.5 rounded-full shadow-lg flex items-center gap-2 max-w-xs">
                                ...
                                {!readOnly && (
                                    <button onClick={() => handleDeleteAnnotation(annotation.id)} className="hover:text-white">
                                        <X size={10} />
                                    </button>
                                )}
                            </div>
        ...
        {activeAnnotationLine !== null && (
            <div className="absolute left-12 z-20 bg-zinc-800 border border-zinc-700 shadow-xl rounded-lg p-3 w-64 animate-in fade-in zoom-in-95"
                ...
                <div className="flex justify-end gap-2">
                    <button onClick={() => setActiveAnnotationLine(null)} className="px-2 py-1 text-xs text-zinc-400 hover:text-white">Cancel</button>
                    <button onClick={handleAddAnnotation} className="px-2 py-1 bg-amber-600 text-white text-xs rounded hover:bg-amber-700">Save</button>
                </div>
            </div>
        )}
      </div>
  ...
  return (
    <div className={`flex flex-col ${readOnly ? 'min-h-0' : 'h-full'}`}>
        ...
        <div className={`flex-1 bg-zinc-900 border border-zinc-800 ${readOnly ? 'rounded-xl border-none overflow-visible' : 'rounded-b-xl overflow-auto'}`}>
            {mode === 'EDIT' ? (
                <textarea
