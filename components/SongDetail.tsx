...
import { Song, SongPart, BandMember, Annotation, SongChart } from '../types';
...
export const SongDetail: React.FC<SongDetailProps> = ({ song, members, availableRoles, onBack, onUpdateSong }) => {
  const [activeTab, setActiveTab] = useState<'OVERVIEW' | 'CHARTS' | 'ASSIGNMENTS' | 'AUDIO'>('OVERVIEW');
  const [activeChartId, setActiveChartId] = useState<string | null>(song.charts.length > 0 ? song.charts[0].id : null);
  ...
  const handleRemoveRole = (memberId: string, role: string) => {
    const newAssignments = song.assignments.filter(a => !(a.memberId === memberId && a.role === role));
    onUpdateSong({ ...song, assignments: newAssignments });
  };
  ...
  const handleUpdateChartContent = (chartId: string, newContent: string) => {
      const updatedCharts = song.charts.map(c => c.id === chartId ? { ...c, content: newContent } : c);
      onUpdateSong({ ...song, charts: updatedCharts });
  };

  const handleUpdateChartAnnotations = (chartId: string, newAnnotations: Annotation[]) => {
      const updatedCharts = song.charts.map(c => c.id === chartId ? { ...c, annotations: newAnnotations } : c);
      onUpdateSong({ ...song, charts: updatedCharts });
  };
    ...
    <div className="h-screen flex flex-col lg:flex-row overflow-hidden bg-zinc-950">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header className="bg-zinc-900 border-b border-zinc-800 p-6 shrink-0">
          <div className="flex items-center justify-between mb-4">
                ...
                {isEditingMetadata ? (
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-white flex items-center gap-3">
                            {song.title}
                            <span className={`text-xs font-normal font-mono px-2 py-1 rounded border ${
                                song.status === 'Performance Ready' ? 'bg-green-900/30 text-green-500 border-green-800' :
                                song.status === 'In Progress' ? 'bg-blue-900/30 text-blue-500 border-blue-800' :
             ...
             <div className="flex items-center gap-4">
                {isEditingMetadata ? (
                    <button onClick={saveMetadata} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2">
                        <Save size={16} /> Save
                    </button>
                ) : (
                    <button onClick={() => setIsEditingMetadata(true)} className="text-zinc-500 hover:text-white p-2">
                        <Edit3 size={18} />
                    </button>
                )}
             </div>
                ...
                <div className="grid lg:grid-cols-2 gap-8">
                    <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    {song.targetDate && (
                                        <div className="bg-zinc-950 p-3 rounded-lg border border-zinc-800 border-l-4 border-l-amber-600">
                                            <span className="text-xs text-zinc-500 uppercase flex items-center gap-1"><Calendar size={12}/> Target Date</span>
                                            <p className="text-xl font-mono text-white">{new Date(song.targetDate).toLocaleDateString()}</p>
                                        </div>
                                    )}
                                </div>
                             )}
                        ...
                        <div className="bg-zinc-900 p-6 rounded-2xl border border-zinc-800">
                            ...
                            <p className="text-zinc-400 leading-relaxed">
                                {song.aiAnalysis || "No analysis generated yet. Upload sheet music or ask the assistant for insights on tone, structure, or specific performance tips."}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-zinc-900 p-6 rounded-2xl border border-zinc-800">
                        ...
                        <div className="space-y-3">
                            {song.assignments.length > 0 ? song.assignments.map((assign, idx) => {
                                const memberName = members.find(m => m.id === assign.memberId)?.name || 'Unknown';
                                return (
                ...
                <div className="h-full flex flex-col gap-6">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-zinc-900 p-2 rounded-xl border border-zinc-800">
                        <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide p-1">
                            {song.charts.map(chart => (
                                ...
                                >
                                    {chart.type === 'GP' ? <Guitar size={14} /> : chart.type === 'PDF' ? <FileText size={14}/> : chart.type === 'IMAGE' ? <File size={14}/> : <Music2 size={14}/>}
                                    {chart.name}
                                </button>
                            ))}
                            <button 
                    ...
                    {isAddingChart && (
                        <div className="bg-zinc-900 border border-zinc-700 p-6 rounded-xl animate-in fade-in zoom-in-95">
                             ...
                             <button onClick={() => setIsAddingChart(false)} className="mt-4 text-zinc-500 hover:text-white text-sm w-full text-center">Cancel</button>
                        </div>
                    )}
                    ...
                    <div className="bg-zinc-900 p-8 rounded-2xl border border-zinc-800">
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            {members.map((member) => {
                                ...
                                const memberAssignments = song.assignments.filter(a => a.memberId === member.id);
                                // Get parts for this member
                                const memberParts = (song.parts || []).filter(p => p.assignedToMemberId === member.id);

                                return (
                                    <div key={member.id} className="bg-zinc-950 border border-zinc-800 rounded-xl p-5 flex flex-col h-full">
                                        <div className="flex items-center gap-4 mb-4 pb-4 border-b border-zinc-900">
                                            <div className={`w-12 h-12 rounded-full ${member.avatarColor || 'bg-zinc-700'} flex items-center justify-center font-bold text-white`}>
                                        ...
                                        <div className="mb-4 flex-1">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {memberAssignments.map((a, idx) => (
                                                    <div key={idx} className="group bg-zinc-900 border border-zinc-800 text-amber-500 px-2 py-1 rounded-md text-sm font-bold flex items-center gap-2">
                                                        <span>{INSTRUMENT_ICONS[a.role] || INSTRUMENT_ICONS['default']} {a.role}</span>
                                                        <button 
                                                ...
                                                {memberAssignments.length === 0 && <p className="text-zinc-600 text-sm italic">No roles assigned.</p>}
                                            </div>
                                        ...
                                        <div className="pt-4 border-t border-zinc-900">
                                             ...
                                             <div className="space-y-1">
                                                ...
                                                {memberParts.length === 0 && <p className="text-zinc-600 text-sm italic">No specific parts assigned.</p>}
                                             </div>
                                        </div>
                                    </div>
                                );
                        ...
                        {audioBlobUrl ? (
                            <>
                                ...
                                <div className="flex justify-center gap-4">
                                    <button 
                                        onClick={() => downloadFile(song.backingTrackUrl!, `${song.title}_backing_track`)}
      ...
      <div className="w-full lg:w-96 bg-zinc-900 border-l border-zinc-800 flex flex-col shrink-0 h-1/2 lg:h-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <div className="flex gap-3">
                 <div className="w-8 h-8 rounded-full bg-amber-900/50 flex items-center justify-center text-xs shrink-0">AI</div>
                 <div className="bg-zinc-800/50 p-3 rounded-r-xl rounded-bl-xl text-sm text-zinc-300 border border-zinc-800">
                    Ready to rock? I can help you analyze tablature, suggest guitar tones for {song.title}, or plan assignments.
                 </div>
            </div>
            ...
            {aiResponse && (
                 <div className="flex gap-3 animate-in fade-in slide-in-from-bottom-2">
                    ...
                    <div className="bg-zinc-800/50 p-3 rounded-r-xl rounded-bl-xl text-sm text-zinc-300 border border-zinc-800 prose prose-invert prose-sm max-w-none">
                        {aiResponse.split('\n').map((line, i) => (
                            <p key={i} className={`mb-1 ${line.startsWith('#') ? 'font-bold text-white mt-2' : ''} ${line.startsWith('-') ? 'pl-4' : ''}`}>
        ...
        <div className="p-4 border-t border-zinc-800 bg-zinc-900">
            <div className="relative">
                ...
                >
                   {loadingAi ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/> : <Mic size={16} />}
                </button>
            </div>
        </div>
      </div>
