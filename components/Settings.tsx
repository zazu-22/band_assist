...
export const Settings: React.FC<SettingsProps> = ({
  ...
  const handleAddMember = () => {
    ...
    const newMember: BandMember = {
      ...
      avatarColor: ['bg-red-500', 'bg-blue-500', 'bg-green-500', 'bg-yellow-500', 'bg-purple-500'][members.length % 5]
    };
  ...
  const handleRemoveMember = (id: string) => {
    if (window.confirm('Are you sure? Assignments for this member in songs will remain but might look orphaned.')) {
  ...
  const handleRemoveRole = (role: string) => {
      if (window.confirm(`Delete role "${role}"? It will still exist on old song assignments but won't be selectable for new ones.`)) {
      ...
      {activeTab === 'ROSTER' && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
          <section className="bg-zinc-900 rounded-2xl border border-zinc-800 p-6">
            ...
            <p className="text-sm text-zinc-400 mb-6">Add everyone in the band. You will assign specific roles (Guitar, Bass, etc.) to them inside each song.</p>
            
            <div className="grid gap-4 mb-6">
              {members.map((member) => (
                <div key={member.id} className="bg-zinc-950 p-4 rounded-xl border border-zinc-800 flex items-center justify-between">
                  {editingId === member.id ? (
                     <div className="flex items-center gap-4 w-full">
                        <div className={`w-10 h-10 rounded-full ${member.avatarColor} flex items-center justify-center font-bold text-white opacity-50`}>
                        ...
                        <input 
                          ...
                          autoFocus
                        />
                        <button onClick={saveEdit} className="p-2 bg-green-900/30 text-green-500 rounded hover:bg-green-900/50"><Check size={18}/></button>
                        <button onClick={() => setEditingId(null)} className="p-2 bg-zinc-800 text-zinc-400 rounded hover:text-white"><X size={18}/></button>
                     </div>
                  ) : (
                    <>
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-full ${member.avatarColor} flex items-center justify-center font-bold text-white shadow-inner`}>
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => startEditing(member)} className="p-2 text-zinc-600 hover:text-white"><Edit2 size={18} /></button>
                        <button onClick={() => handleRemoveMember(member.id)} className="p-2 text-zinc-600 hover:text-red-500"><Trash2 size={18} /></button>
                      </div>
                    </>
                  )}
                </div>
            ...
            <div className="flex gap-3">
              ...
              />
              <button onClick={handleAddMember} className="bg-amber-600 hover:bg-amber-700 text-white px-6 py-2 rounded-lg font-bold transition-colors">
                Add Person
              </button>
            </div>
          </section>
      ...
      {activeTab === 'ROLES' && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            <section className="bg-zinc-900 rounded-2xl border border-zinc-800 p-6">
                ...
                <p className="text-sm text-zinc-400 mb-6">Define the list of instruments or roles that can be assigned in songs (e.g., "Lead Guitar", "Cowbell", "Manager").</p>

                <div className="flex flex-wrap gap-3 mb-8">
                    {availableRoles.map((role) => (
                        <div key={role} className="group bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 flex items-center gap-2">
                ...
                <div className="flex gap-3 max-w-md">
                    ...
                    />
                    <button onClick={handleAddRole} className="bg-zinc-800 hover:bg-zinc-700 text-white px-4 py-2 rounded-lg font-bold transition-colors border border-zinc-700">
                        Add Role
                    </button>
                </div>
            </section>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
              <section className="bg-zinc-900 rounded-2xl border border-zinc-800 p-6">
                ...
                <p className="text-sm text-zinc-400 mb-6">
                    Export your band's data to a JSON file. You can email this file to your brothers so they can load it on their devices.
                    This is the safest way to ensure everyone has the same setlist and assignments.
                </p>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    <div className="bg-zinc-950 p-6 rounded-xl border border-zinc-800 flex flex-col items-center text-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-zinc-500 mt-1">Save current songs, members, assignments, and schedule.</p>
                        </div>
                        <button 
                    ...
                    <div className="bg-zinc-950 p-6 rounded-xl border border-zinc-800 flex flex-col items-center text-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-zinc-500 mt-1">Restore from a backup file. <span className="text-red-400 font-bold">Overwrites current data.</span></p>
                        </div>
              ...
              <section className="bg-red-900/10 rounded-2xl border border-red-900/30 p-6 flex items-start gap-4">
                 ...
                 <div>
                    ...
                    <p className="text-sm text-red-300/70 mt-1">
                        Browser storage is limited (usually around 5MB). If you upload too many large PDFs or Audio files, 
                        the app may stop saving automatically. We recommend using the "Export Project" feature frequently
