...
import { Calendar, Clock, MapPin, Plus, Trash2, Guitar, Mic2, Star, Edit2 } from 'lucide-react';
...
export const ScheduleManager: React.FC<ScheduleManagerProps> = ({ events, setEvents, songs, onNavigateToSong }) => {
  ...
  const timelineItems = [
    ...
    ...songTargets.map(t => ({ ...t, isEvent: false }))
  ].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());

  const getEventIcon = (type: string) => {
    switch (type) {
        case 'PRACTICE': return <Guitar className="text-blue-500" />;
        case 'GIG': return <Star className="text-amber-500 fill-amber-500" />;
        case 'DEADLINE': return <Clock className="text-red-400" />;
        default: return <Calendar className="text-zinc-500" />;
    }
  };

  const getEventColor = (type: string) => {
      switch(type) {
          case 'PRACTICE': return 'border-blue-500/30 bg-blue-500/5';
          case 'GIG': return 'border-amber-500/50 bg-amber-500/10';
          case 'DEADLINE': return 'border-zinc-800 bg-zinc-900/50 dashed border-2';
    ...
    <div className="p-6 lg:p-10 max-w-5xl mx-auto">
      ...
      {isAdding && (
        <div className="bg-zinc-900 border border-zinc-700 p-6 rounded-2xl mb-8 animate-in fade-in slide-in-from-top-4 shadow-2xl">
            <h3 className="text-lg font-bold text-white mb-4">{editingId ? 'Edit Event' : 'New Event'}</h3>
            ...
            <div className="flex justify-end gap-3">
                <button onClick={resetForm} className="px-4 py-2 text-zinc-400 hover:text-white">Cancel</button>
                <button onClick={handleSaveEvent} className="px-6 py-2 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-lg">
        ...
        {timelineItems.length === 0 ? (
            ...
            <div className="relative pl-4 md:pl-8 border-l-2 border-zinc-800 space-y-8 py-4">
                {timelineItems.map((item: any) => {
                    const dateObj = new Date(item.date + (item.time ? `T${item.time}` : 'T00:00'));
                    const isPast = dateObj < new Date();
                    
                    return (
                        <div key={item.id} className={`relative ${isPast ? 'opacity-50 grayscale' : ''}`}>
                            {/* Timeline Dot */}
                            <div className={`absolute -left-[25px] md:-left-[41px] top-4 w-5 h-5 rounded-full border-4 border-zinc-950 ${item.type === 'GIG' ? 'bg-amber-500' : item.type === 'DEADLINE' ? 'bg-red-500' : 'bg-blue-500'}`}></div>
                            
                            <div className={`group relative p-5 rounded-2xl border ${getEventColor(item.type)} flex flex-col md:flex-row gap-4 md:items-center transition-all hover:border-zinc-600`}>
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-1">
                                        {getEventIcon(item.type)}
                                        <span className="text-xs font-bold uppercase tracking-wider text-zinc-500">{item.type}</span>
                                    </div>
                                    <h3 
                                    ...
                                    {item.notes && <p className="text-zinc-500 text-sm mt-2 whitespace-pre-wrap">{item.notes}</p>}
                                </div>
                                ...
                                <div className="flex flex-row md:flex-col items-center md:items-end justify-between gap-2 md:gap-0 border-t md:border-t-0 md:border-l border-zinc-800/50 pt-3 md:pt-0 md:pl-6 shrink-0 min-w-[120px]">
                                    <div className="text-right">
                                        <div className="text-2xl font-bold text-white leading-none">{dateObj.getDate()}</div>
                                        <div className="text-sm text-zinc-500 uppercase font-bold">{dateObj.toLocaleDateString('en-US', { month: 'short' })}</div>
                                        {item.time && <div className="text-xs text-zinc-600 font-mono mt-1">{item.time}</div>}
                                    </div>
