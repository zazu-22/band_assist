...
import { GripVertical, Plus, Clock, Trash2, Play } from 'lucide-react';
...
export const SetlistManager: React.FC<SetlistManagerProps> = ({ songs, setSongs, onSelectSong, onStartGig }) => {
  ...
  const DropIndicator = ({ index }: { index: number }) => (
    ...
    >
        <span className="text-amber-500/50 text-sm font-bold uppercase tracking-widest pointer-events-none">Drop Here</span>
    </div>
  );
      ...
      {isAdding && (
        <div className="mb-6 bg-zinc-900 p-4 rounded-xl border border-zinc-700 animate-in fade-in slide-in-from-top-4">
            ...
            <div className="flex gap-3">
                ...
                />
                <button onClick={handleAddSong} className="px-4 py-2 bg-zinc-100 text-zinc-900 font-bold rounded-lg">Add</button>
                <button onClick={() => setIsAdding(false)} className="px-4 py-2 text-zinc-400 hover:text-white">Cancel</button>
            </div>
        </div>
      )}
                ...
                <React.Fragment key={song.id}>
                    ...
                    >
                        ...
                        <div className="flex-1 cursor-pointer" onClick={() => onSelectSong(song.id)}>
                            <h4 className="font-bold text-zinc-100 group-hover:text-amber-500 transition-colors">{song.title}</h4>
                            <p className="text-xs text-zinc-500">{song.key} • {song.bpm} BPM</p>
                        </div>
                        <div className="hidden md:block text-sm font-mono text-zinc-400 px-4 select-none">
                            {song.duration}
                        </div>
                        <div className="flex items-center gap-2">
                             <span className={`text-[10px] uppercase px-2 py-0.5 rounded border select-none ${
                                song.status === 'Performance Ready' ? 'border-green-800 text-green-500' :
                                song.status === 'In Progress' ? 'border-blue-800 text-blue-500' :
